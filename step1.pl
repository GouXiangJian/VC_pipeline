...
my $VERSION = 'variant (SNP/INDEL) calling, step1 v1.2 (2025-07-22)';
...
Notes:
  ...
  (d) All files and directories provided as parameters should be absolute paths.
  (e) Remember to build a genomic library and create index (.fai/.dict) before using the script.
      e.g.
...
foreach my $dir (glob "$germplasmDir/*") {
    ...
    lsfInfo($oEACH, $germplasm, $germplasmDir, $outputDir, $softwareDir, $queue, $thread, $memory, $refGenomeLib, $refGenomeFa, $jumpNGSQC, $jumpBWA, $jumpSortSam, $jumpMarkDuplicates, $jumpHaplotypeCaller, $mbq);
    close $oEACH;
...
sub lsfInfo {
    my ($handle, $germplasm, $germplasmDir, $outputDir, $softwareDir, $queue, $thread, $memory, $refGenomeLib, $refGenomeFa, $jumpNGSQC, $jumpBWA, $jumpSortSam, $jumpMarkDuplicates, $jumpHaplotypeCaller, $mbq) = @_;
...
$jumpHaplotypeCaller $softwareDir/gatk-4.1.2.0/gatk --java-options "-Xmx${memory}G" HaplotypeCaller -R $refGenomeFa -I $outputDir/MarkDuplicatesOut/$germplasm.sort.mark.bam -O $outputDir/HaplotypeCallerOut_nonBQSR/$germplasm.g.vcf -ERC GVCF -stand-call-conf 30 -mbq $mbq --native-pair-hmm-threads 20
